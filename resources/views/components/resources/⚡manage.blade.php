        ...
        <div class="glass-card rounded-3xl p-6 mb-8">
            <form wire:submit.prevent="{{ $editingId ? 'update' : 'create' }}" class="grid gap-4 md:grid-cols-2">
                ...
                <div class="md:col-span-2 flex gap-2">
                    ...
                    @if($editingId)
                        <button type="button" wire:click="$reset('title', 'url', 'type', 'editingId')" class="bg-slate-200 text-slate-700 px-5 py-2 rounded-full font-semibold">Annuler</button>
                    @endif
                </div>
            </form>
        </div>
                    ...
                    @foreach($resources as $resource)
                        <li class="py-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                            ...
                            <div class="flex gap-2">
                                <button wire:click="edit({{ $resource->id }})" class="px-4 py-2 rounded-full bg-amber-400 text-white font-semibold hover:bg-amber-500 transition">Éditer</button>
