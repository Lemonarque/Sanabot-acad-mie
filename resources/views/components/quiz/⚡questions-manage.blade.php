<div class="min-h-screen aurora-bg py-10">
    <div class="max-w-4xl mx-auto px-6">
        <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-8">
            ...
            <a href="{{ url()->previous() }}" class="chip px-4 py-2 rounded-full text-sm">Retour au quiz</a>
        </div>

        <div class="glass-card rounded-3xl p-6 mb-8">
            <form wire:submit.prevent="{{ $editingId ? 'update' : 'create' }}" class="grid gap-4">
                <div>
                    <label class="block text-sm font-medium">Intitulé de la question</label>
                    <input type="text" wire:model.defer="question_text" class="mt-1 block w-full border-slate-200 rounded-xl px-4 py-2" required>
                    @error('question_text') <span class="text-rose-500 text-xs">{{ $message }}</span> @enderror
                </div>
                <div class="flex gap-2">
                    ...
                    @if($editingId)
                        <button type="button" wire:click="$reset('question_text', 'editingId')" class="bg-slate-200 text-slate-700 px-5 py-2 rounded-full font-semibold">Annuler</button>
                    @endif
                </div>
            </form>
        </div>
                ...
                <ul class="divide-y divide-slate-200">
                    @foreach($questions as $question)
                        <li class="py-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                            ...
                            <div class="flex gap-2">
                                <button wire:click="edit({{ $question->id }})" class="px-4 py-2 rounded-full bg-amber-400 text-white font-semibold hover:bg-amber-500 transition">Éditer</button>
                                ...
                                <a href="{{ route('quiz.answers.manage', ['questionId' => $question->id]) }}" class="px-4 py-2 rounded-full bg-sky-500 text-white font-semibold hover:bg-sky-600 transition">Réponses</a>
                            </div>
                        </li>
                    @endforeach
                </ul>
